...
// Format: { THEME_NAME = { light: "", dark: ".dark" } as const
...
${colorConfig
  .map(([key, itemConfig]) => {
    const color =
      ...
      itemConfig.color
    return color ? `  --color-${key}: ${color};`  = RechartsPrimitive.Tooltip
          ...
          {payload.map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={cn(
                  "flex w-full flex-wrap items-stretch gap-2 [&>svg] === "dot" && "items-center"
                )}
              >
...
const ChartLegendContent = React.forwardRef & {
      payload? = false, payload = [], verticalAlign = "bottom", nameKey },
    ref
  ) => {
    const { config } = useChart()
        ...
        {payload.map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "flex items-center gap-1.5 [&>svg] ="h-2 w-2 shrink-0 rounded-[2px]"
                  style={{
                    backgroundColor = "ChartLegend"

// Helper to extract item config from a payload.
function getPayloadConfigFromPayload(
  config == "object" || payload === null) {
    return undefined
  }
