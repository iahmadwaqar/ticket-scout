...
export default function CollapsibleLogPanel({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute top-0 left-0 right-0 h-1 bg-border hover:bg-primary/50 cursor-ns-resize',
      ...
      <ScrollArea
        className="p-4"
        style={{
          height: `calc(${panelHeight}vh - 120px)` // Subtract header and filter heights
        }}
      >
        {logs.length === 0 ? (
          ...
          <div className="space-y-2">
            {filteredLogs.map((log) => (
              <div
                key={log.id}
                className="flex items-start gap-3 p-3 transition-colors border rounded-lg bg-card hover:bg-muted/50"
              >
