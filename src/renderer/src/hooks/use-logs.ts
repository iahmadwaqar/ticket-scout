...
export function useLogs() {
  ...
  const getLogsBySeverity = useCallback((severity: LogEntry['severity']) => {
    return logs.filter(log => log.severity === severity)
  }, [logs])

  // Filter logs by profile ID
  const getLogsByProfile = useCallback((profileId: string) => {
    return logs.filter(log => log.profileId === profileId)
  }, [logs])

  // Get recent logs (last N entries)
  const getRecentLogs = useCallback((count: number = 100) => {
  ...
  useEffect(() => {
    ...
    if (window.api.onLogAdded && typeof window.api.onLogAdded === 'function') {
      ...
      const unsubscribe = window.api.onLogAdded((newLog: LogEntry) => {
        setLogs(prevLogs => {
