  ...
  try {
    ...
    if (toast.duration !== undefined) {
      if (typeof toast.duration !== 'number' || toast.duration < 0) {
        ...
        sanitizedToast.duration = Math.max(0, Math.min(10000, toast.duration)) // Cap at 10 seconds
      }
    }
...
export function validateLaunchAllConfig(config) {
  try {
    ...
    if (
      typeof config.startProfile !== 'number' ||
      !Number.isInteger(config.startProfile) ||
      config.startProfile < 0
    ) {
    ...
    if (
      typeof config.profileCount !== 'number' ||
      !Number.isInteger(config.profileCount) ||
 ...
 * - Runtime validation for profile objects
...
export const ValidationResultSchema = Object.freeze({
  ...
  sanitizedData: 'any?' // optional
})

/**
 * Profile field validation rules structure
 */
export const ProfileFieldRulesSchema = Object.freeze({
  required: 'boolean',
...
const PROFILE_VALIDATION_SCHEMA = {
  ...
  status: {
    required: true,
    type: 'string',
    allowedValues: ['Launching', 'Ready', 'LoggedIn', 'Navigating', 'Scraping', 'SearchingTickets', 'RandomBrowsing', 'InQueue', 'WaitingForCaptcha', 'SessionExpired', 'RateLimited', 'Error', 'Stopping', 'Idle', 'Success', 'Stopped']
  },
  loginState: {
    required: true,
    type: 'string',
    allowedValues: Object.values(LOGIN_STATE)
  },
  supporterId: {
    required: true,
    type: 'string',
  ...
  priority: {
    required: true,
    type: 'string',
    allowedValues: Object.values(PRIORITY_LEVEL)
  },
  ticketCount: {
    required: true,
    type: 'number',
  ...
  operationalState: {
    required: true,
    type: 'string',
    allowedValues: ['idle', 'active', 'stopping', 'error']
  },
  launchedAt: {
    required: false,
    type: 'string',
 ...
 * @returns Validation result with sanitized data
 */
function validateProfileField(fieldName, value, rules) {
  try {
...
/**
 * Check if an object is a valid Profile
...
/**
 * Check if an object is a valid EnhancedProfile
 * @param obj - Object to check
 * @returns True if object is a valid EnhancedProfile
 */
export function isEnhancedProfile(obj) {
  if (!isProfile(obj)) {
    return false
  }

  const enhancedFields = ['ticketCount', 'lastActivity', 'operationalState']
  ...
  try {
    ...
    for (const [fieldName, value] of Object.entries(updates)) {
      if (fieldName in PROFILE_VALIDATION_SCHEMA) {
        const rules = PROFILE_VALIDATION_SCHEMA[fieldName]
        const result = validateProfileField(fieldName, value, rules)

        if (!result.isValid) {
          errors.push(result.error)
        } else {
          sanitizedUpdates[fieldName] = result.sanitizedData
        }
      } else {
  ...
  try {
    ...
    for (const [fieldName, rules] of Object.entries(PROFILE_VALIDATION_SCHEMA)) {
      ...
      if (!result.isValid) {
        ...
        sanitizedProfile[fieldName] = result.sanitizedData
      }
    }
 ...
 * @returns Validation result with sanitized data
 */
export function validateLogEntry(profileId, severity, message) {
  try {
    const errors = []
    const sanitizedData = {}
