 ...
 * - OperationResult interface for consistent return values
...
import type { ProfileStatus } from '../../renderer/src/types'
import { sendToast } from '../service-handlers'
import { BrowserWindow } from 'electron'

// Operation result interface for consistent return values
...
export interface ProfileOperations {
  launchProfile(profileId: string): Promise<OperationResult>
  stopProfile(profileId: string): Promise<OperationResult>
  closeProfile(profileId: string): Promise<OperationResult>
}
...
const activeGologinInstances: Map<string, { gologin: any; result: any }> = new Map()
 ...
 * @returns OperationResult if validation fails, null if validation passes
 */
 ...
 */
export async function launchProfile(profileId: string): Promise<OperationResult> {
 ...
 * @returns OperationResult with success status and messages
 */
export async function stopProfile(profileId: string): Promise<OperationResult> {
  const operationStartTime = Date.now()
 ...
 * @returns OperationResult with success status and messages
 */
export async function closeProfile(profileId: string): Promise<OperationResult> {
  const operationStartTime = Date.now()
 ...
 */
export function getActiveInstances(): Map<string, { gologin: any; result: any }> {
