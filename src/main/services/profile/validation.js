...
const PROFILE_VALIDATION_SCHEMA = {
  ...
  status: {
    required: true,
    type: 'string',
    allowedValues: Object.values(PROFILE_STATUS)
  },
  loginState: {
    required: true,
    type: 'string',
  ...
  operationalState: {
    required: true,
    type: 'string',
    allowedValues: Object.values(OPERATIONAL_STATE)
  },
  launchedAt: {
    required: false,
    type: 'string',
...
export function validateLaunchAllConfig(config) {
  try {
    ...
    if (typeof config.start !== 'number' || !Number.isInteger(config.start) || config.start < 0) {
      errors.push('start must be a positive integer')
    } else {
      sanitizedConfig.start = Math.max(0, Math.floor(config.start))
    }
